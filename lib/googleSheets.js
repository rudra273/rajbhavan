 ...
 * Columns: id, title, cloudinary_url, category, is_featured, location, total_area, floors, price, package, duration, gallery_images
 */
export async function getProjects() {
    ...
    return data.map(project => ({
        ...
        gallery_images: project.gallery_images
            ? project.gallery_images.split(",").map(s => s.trim()).filter(Boolean)
            : []
    }));
}
            ...
            values: [
                [
                    ...
                    Array.isArray(projectData.gallery_images)
                        ? projectData.gallery_images.join(",")
...
export async function updateProject(id, projectData) {
    ...
    await sheets.spreadsheets.values.update({
        spreadsheetId: process.env.GOOGLE_SHEET_ID,
        range: `Projects!A${sheetRowNumber}:L${sheetRowNumber}`,
        valueInputOption: "USER_ENTERED",
        requestBody: {
            values: [
                [
 ...
 * Columns: id, name, review_text, date
 */
