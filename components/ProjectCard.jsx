...
export default function ProjectCard({ id, title, cloudinary_url, category, location, total_area, floors, price }) {
    const stats = [
        ...
        { label: "Price", value: price, icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23" /><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6" /></svg> },
    ].filter(s => s.value);
    ...
    return (
        <>
            <style>{`
                ...
                .proj-card { position: relative; display: block; overflow: hidden; background: #0a0f1a; text-decoration: none; }
                ...
                .proj-base { position: absolute; bottom: 0; left: 0; right: 0; z-index: 2; padding: 24px; transform: translateY(0); transition: transform 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94); }
                ...
                .proj-card:hover .proj-stats { max-height: 120px; opacity: 1; margin-top: 14px; }
                .proj-divider { width: 24px; height: 1px; background: rgba(255,255,255,0.25); margin: 10px 0 0; transition: width 0.4s ease; }
                .proj-card:hover .proj-divider { width: 100%; background: rgba(255,255,255,0.12); }
                .proj-arrow { display: inline-flex; align-items: center; justify-content: center; width: 32px; height: 32px; border: 1px solid rgba(255,255,255,0.2); border-radius: 50%; opacity: 0; transform: translateX(-6px); transition: opacity 0.3s ease 0.1s, transform 0.3s ease 0.1s, border-color 0.2s; }
            ...
            `}</style>
            <Link href={`/projects/${id}`} className="proj-card group" style={{ display: "block", textDecoration: "none" }}>
                {/* Image */}
