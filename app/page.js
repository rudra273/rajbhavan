...
export default async function Home() {
  ...
  return (
    <>
      ...
      {/* Shop / Hardware Teaser Section */}
      <section style={{ background: "#0a0f1a", padding: "80px 24px" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "60px", alignItems: "center" }}>
          <div>
            ...
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "14px", color: "#94a3b8", lineHeight: 1.7, margin: "0 0 28px", maxWidth: "440px" }}>
            ...
            <div style={{ display: "flex", gap: "16px", flexWrap: "wrap", marginBottom: "32px" }}>
              {["Door Handles", "Locks", "Hinges", "Cabinet Hardware", "Bathroom Fittings", "Sliding Systems"].map(tag => (
                <span key={tag} style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "11px", fontWeight: 600, color: "#64748b", background: "#1e293b", padding: "5px 12px", borderRadius: "2px", letterSpacing: "0.03em" }}>{tag}</span>
              ))}
            </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1px", background: "#1e293b", border: "1px solid #1e293b" }}>
            {[
              { name: "Door Handles & Knobs", icon: "🚪" },
              { name: "Mortise & Smart Locks", icon: "🔒" },
              { name: "Hinges & Pivots", icon: "🔩" },
              { name: "Sliding Door Systems", icon: "↔️" },
              { name: "Cabinet Hardware", icon: "🗄️" },
              { name: "Bathroom Fittings", icon: "🚿" },
            ].map(item => (
              <div key={item.name} style={{ background: "#0f172a", padding: "24px 20px", display: "flex", alignItems: "center", gap: "12px" }}>
                <span style={{ fontSize: "20px", lineHeight: 1 }}>{item.icon}</span>
                <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "12px", fontWeight: 500, color: "#cbd5e1", lineHeight: 1.4 }}>{item.name}</span>
              </div>
            ))}
          </div>
        </div>
